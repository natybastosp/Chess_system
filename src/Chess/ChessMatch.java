...
public class ChessMatch {
	...
	public boolean [][]possibleMoves(ChessPosition sourcePosition){
		...
		valideteSoucePosition(position);
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		valideteSoucePosition(source);
		valideteTargetPosition(source,target);
		Piece  capturedPiece = makeMove(source,target);
		
		if(testCheck(currentPlayer)) {
			undoMove(source, target, capturedPiece);
			throw new ChessExcepection("You can't put yourself in check");
		}
	...
	private void valideteSoucePosition(Position position){
		if (!board.thereIsAPiece(position)) {
			throw new ChessExcepection("There is no piece on source position");
		}
		if (currentPlayer != ((ChessPiece)board.piece(position)).getColor()) {
			throw new ChessExcepection("The chosen piece is not your");
		}
		if(!board.piece(position).isTheAnyPossibleMove()) {
			throw new ChessExcepection("There is no possible moves for the chosen  piece");  			
		}
	}
	
	private void valideteTargetPosition(Position source, Position target) {
		if(!board.piece(source).pissobleMoves(target)) {
			throw new ChessExcepection("The chosen piece can't move to target position");
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("Therwe is no " + color +" king on the board");
	}
	
	private boolean testCheck(Color color) {
		Position kingPosition = king(color).getChessPosition().toPosition();
		List<Piece> opponentPiece = pieceOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == opponente(color)).collect(Collectors.toList());
